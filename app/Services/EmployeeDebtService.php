...
class EmployeeDebtService
{
    ...
    public function planFrom(Collection $ofMonth, string $month, float $bonusOfMonth): array
    {
        ...
        // Бонус, ещё не разобранный: из бонуса месяца ВЫЧИТАЕМ уже удержанное —
        // иначе план после прогона cron распределял бы тот же бонус второй раз,
        // и суммарное удержание (факт + план) превышало бы бонус месяца,
        // залезая в оклад — а оклад не трогается никогда.
        $budget = max(0.0, round($bonusOfMonth - $charged, 2));
        $perDebt = [];

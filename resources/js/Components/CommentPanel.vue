    ...
    <div class="space-y-4">
        <div class="flex gap-2">
            <textarea v-model="form.body" rows="2" placeholder="Написать комментарий…"
                class="w-full rounded-md border-slate-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500"></textarea>
        </div>
        <div><PrimaryButton :disabled="form.processing || !form.body" @click="add">Отправить</PrimaryButton></div>

        <div class="space-y-3">
            <div v-for="c in comments" :key="c.id" class="rounded-md bg-slate-50 p-3 text-sm">
                <div class="flex items-center justify-between">
                    <span class="font-medium text-slate-800">{{ c.user?.name }}</span>
            ...
            <div v-if="!comments.length" class="py-4 text-center text-sm text-slate-400">Комментариев нет</div>
        </div>
    </div>
</template>

...
<template>
    <div class="space-y-2">
        <div v-for="t in tasks" :key="t.id" class="flex items-center justify-between rounded-md px-3 py-2 text-sm"
            :class="isPastDue(t.due_date, t.status==='done') ? 'bg-red-50 ring-1 ring-red-200' : 'bg-slate-50'">
            <div class="min-w-0">
            ...
            <div class="flex items-center gap-2">
                ...
                <button class="text-slate-400 hover:text-indigo-600" title="Редактировать" @click="openEdit(t)">✎</button>
        ...
        <div v-if="adding" class="rounded-md border border-dashed p-3">
            ...
            <div class="mb-2 grid grid-cols-2 gap-2">
                <select v-model="form.assignee_id" class="rounded-md border-slate-300 text-sm">
                    <option value="">Исполнитель…</option>
                    <option v-for="u in users" :key="u.id" :value="u.id">{{ u.name }}</option>
                </select>
                <select v-model="form.priority" class="rounded-md border-slate-300 text-sm">
            ...
            <div class="flex gap-2">
                ...
                <button class="text-sm text-slate-500" @click="adding = false">Отмена</button>
            </div>
        </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div>
                            ...
                            <select v-model="editForm.assignee_id" class="mt-1 w-full rounded-md border-slate-300 text-sm">
                                <option value="">—</option>
                                <option v-for="u in users" :key="u.id" :value="u.id">{{ u.name }}</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select v-model="editForm.priority" class="mt-1 w-full rounded-md border-slate-300 text-sm">
                                <option value="low">Низкий</option><option value="medium">Средний</option><option value="high">Высокий</option>
                            </select>
                        </div>
                        <div>
                            <InputLabel value="Статус" />
                            <select v-model="editForm.status" class="mt-1 w-full rounded-md border-slate-300 text-sm">
                    ...
                    <div><InputLabel value="Заметка" /><textarea v-model="editForm.note" rows="2" class="mt-1 w-full rounded-md border-slate-300 text-sm"></textarea></div>
                    <div><InputLabel value="Описание" /><textarea v-model="editForm.description" rows="2" class="mt-1 w-full rounded-md border-slate-300 text-sm"></textarea></div>
                </div>

...
<template>
    ...
    <AppLayout>
        ...
        <PageLayout title="Профиль" subtitle="личная карточка сотрудника" :wide="false">
            <div class="mx-auto max-w-3xl space-y-6">
                <div v-if="isAdmin" class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    <label class="mb-1 block text-[11px] uppercase tracking-wide text-slate-400">Сотрудник</label>
                    <select v-model="selectedId" @change="loadUser" class="w-full rounded-md border-slate-300 text-sm shadow-sm focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20">
                        <option v-for="e in employees" :key="e.id" :value="e.id">{{ e.name }}</option>
                    </select>
                </div>

                <!-- Секция §6: карточка профиля -->
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex items-center gap-4">
                        <div class="relative flex-shrink-0">
                            <span class="flex h-20 w-20 items-center justify-center overflow-hidden rounded-full bg-indigo-600 text-2xl font-bold text-white shadow-sm">
                            ...
                            <template v-if="isSelf">
                                ...
                                <button @click="avatarInput?.click()" :disabled="avatarForm.processing" title="Сменить фото"
                                    class="absolute -bottom-1 -right-1 flex h-7 w-7 items-center justify-center rounded-full border-2 border-white bg-indigo-600 text-white shadow-sm transition-colors duration-150 hover:bg-indigo-700 disabled:opacity-50">
                        ...
                        <div>
                            <div class="text-lg font-semibold text-slate-900">{{ current.name }}</div>
                            <div class="mt-1.5 flex flex-wrap items-center gap-2">
                                <span class="rounded-full bg-indigo-50 px-2.5 py-0.5 text-xs font-medium text-indigo-700">{{ roleLabels[current.role] ?? current.role ?? '—' }}</span>
                    ...
                    <template v-if="isAdmin">
                        <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select v-model="form.department_id" class="mt-1 w-full rounded-md border-slate-300 text-sm shadow-sm focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20">
                                    <option value="">—</option>
                    ...
                    <dl v-else class="mt-6 grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
                        <div><dt class="text-[11px] uppercase tracking-wide text-slate-400">Email</dt><dd class="mt-0.5 text-slate-800">{{ current.email }}</dd></div>
                        <div><dt class="text-[11px] uppercase tracking-wide text-slate-400">Телефон</dt><dd class="mt-0.5 text-slate-800">{{ current.phone || '—' }}</dd></div>
                        <div><dt class="text-[11px] uppercase tracking-wide text-slate-400">Отдел / должность</dt><dd class="mt-0.5 text-slate-800">{{ deptName(current.department_id) }}</dd></div>
                        <div><dt class="text-[11px] uppercase tracking-wide text-slate-400">Роль</dt><dd class="mt-0.5 text-slate-800">{{ roleLabels[current.role] ?? current.role ?? '—' }}</dd></div>
                    </dl>
                </div>

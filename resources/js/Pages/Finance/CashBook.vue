    ...
    <AppLayout>
        <template #header>
            <div class="flex w-full min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <span class="truncate">{{ isAll ? 'Все деньги · отчёт за день' : (isBank ? 'Банк · отчёт за день' : 'Касса · отчёт за день') }}</span>
        ...
        <div class="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                <div v-for="op in operations" :key="op.id" class="flex items-center gap-3 px-4 py-3 transition hover:bg-slate-50">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-2">
                            ...
                            <span v-if="isAll" class="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold"
            ...
            <div v-else class="px-4 py-14 text-center">
                ...
                <div class="text-sm text-slate-400">За {{ dayLabel }} движения денег не было</div>
            </div>

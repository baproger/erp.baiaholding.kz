...
Route::middleware('auth')->group(function () {
    ...
    // История изменений лота (кто что сделал) — для модалки-глаза.
    Route::get('pre-deals/{preDeal}/history', [\App\Http\Controllers\PreDealController::class, 'history'])->name('preDeals.history');
    ...
    // Долги сотрудников: гасятся автоматически из бонусов (debts:charge).
    Route::post('payroll/debts', [PayrollController::class, 'storeDebt'])->name('payroll.debts.store');
    Route::delete('payroll/debts/{debt}', [PayrollController::class, 'destroyDebt'])->name('payroll.debts.destroy');
    ...
    // Ошибки браузера (JS) — в общий журнал Аудит → Ошибки.
    Route::post('client-errors', [\App\Http\Controllers\ClientErrorController::class, 'store'])->middleware('throttle:30,1')->name('clientErrors.store');
